...
public class Day12Part2
{
    ...
    private IEnumerable<IReadOnlyCollection<string>> GetAllPaths(IReadOnlyCollection<string> path, string current, IReadOnlyDictionary<string, List<string>> nodes)
    {
        if (IsLowercase(current) && path.Contains(current))
        {
            ...
            {
                return nodes[current].SelectMany(next => GetAllPaths(path.Concat(new []{current + "_alt"}).ToList(), next, nodes));
            }
            return new[] {path};
        }
    ...
    private class Tests
    {
        ...
        public void TestData2()
        {
            ...
            var sut = new Day12Part2();
            Assert.AreEqual(103, sut.Run(data));
        }
        ...
        public void TestData3()
        {
            ...
            var sut = new Day12Part2();
            Assert.AreEqual(3509, sut.Run(data));
        }
        ...
        public void Data()
        {
            ...
            var sut = new Day12Part2();
            Assert.AreEqual(99138, sut.Run(data));
        }
    }
}