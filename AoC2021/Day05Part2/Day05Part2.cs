...
public class Day05Part2
{
    ...
    private class Tests
    {
        ...
        public void GetPositions()
        {
            ...
            Assert.That(sut.GetPositions(new[] { new[] { 1, 1 }, new[] { 3, 3 } }),
                Is.EqualTo(new List<(int, int)> { (1, 1), (2, 2), (3, 3) }));
            Assert.That(sut.GetPositions(new[] { new[] { 9, 7 }, new[] { 7, 9 } }),
                Is.EqualTo(new List<(int, int)> { (9, 7), (8, 8), (7, 9) }));
            Assert.That(sut.GetPositions(new[] { new[] { 1, 0 }, new[] { 4, 3 } }),
                Is.EqualTo(new List<(int, int)> { (1, 0), (2, 1), (3, 2), (4, 3) }));
            Assert.That(sut.GetPositions(new[] { new[] { 0, 1 }, new[] { 3, 4 } }),
                Is.EqualTo(new List<(int, int)> { (0, 1), (1, 2), (2, 3), (3, 4) }));
            Assert.That(sut.GetPositions(new[] { new[] { 3, 4 }, new[] { 0, 1 } }),
                Is.EqualTo(new List<(int, int)> { (3, 4), (2, 3), (1, 2), (0, 1) }));
            Assert.That(sut.GetPositions(new[] { new[] { 0, 9 }, new[] { 5, 9 } }),
                Is.EqualTo(new List<(int, int)> { (0, 9), (1, 9), (2, 9), (3, 9), (4, 9), (5, 9) }));
        }
        ...
        public void Data()
        {
            ...
            var sut = new Day05Part2();
            Assert.That(sut.Run(data), Is.EqualTo(20121));
        }
    }
}